...
argocd app create playground-app \
  ...
  --dest-namespace dev \
  --sync-policy automated  # 🔄 Création de l'application avec synchronisation automatique activée
...
kubectl wait --for=condition=created pod -n dev -l app=will-playground --timeout=300s
